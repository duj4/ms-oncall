...
func TestSimpleNotifications(t *testing.T) {
	...
	sql := `
	update config_limits set max = -1
	where id in ('contact_methods_per_user', 'notification_rules_per_user');
	...
	values
		({{uuid "sid"}}, 'testing', 'auto:1:smoke:simplenotification_test:1:1');
