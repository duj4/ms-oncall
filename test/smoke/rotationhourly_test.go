...
func TestRotation_Hourly(t *testing.T) {
	...
	values
		({{uuid "rot1"}}, 'default rotation', 'hourly', now(), 1, 'America/Chicago', {{smokeOrganizationID}});
	...
	insert into alerts (service_id, summary, dedup_key) values
		({{uuid "sid"}}, 'testing', 'auto:1:smoke:rotationhourly_test:1:1');
